...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private void corkscrew(int n, GL2 gl2) {
        ...
        for (float i = 0; i < 100 * n; i++) {
            x = Math.cos(i * angle) * (0.01 * i);
            y = Math.sin(i * angle) * (0.01 * i);
            z = i / (2 * n);
    ...
    private void drawTriangle(GL2 gl2, float j, float n, float k) {
        gl2.glColor3f(1, 0, 1);
        gl2.glBegin(GL2.GL_TRIANGLES);

        gl2.glVertex3f((float) (Math.cos(j * 2 * Math.PI / n)), (float) (Math.sin(j * 2 * Math.PI / n)), 2f);
        gl2.glVertex3f((float) (Math.cos(k * 2 * Math.PI / n)), (float) (Math.sin(k * 2 * Math.PI / n)), 2f);
        gl2.glVertex3f(0, 0, 0);

        gl2.glEnd();
    }

    private void drawPiramid(int n, GL2 gl2) {
        gl2.glScalef(2, 2, 2);
        gl2.glRotatef(45, 0, 1, 0);

        for (int j = 0; j < n; j++) {
            drawTriangle(gl2, j - 1, n, j);
        }
    }
